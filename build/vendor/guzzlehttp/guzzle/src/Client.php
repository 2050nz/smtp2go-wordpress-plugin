...
use SMTP2GOWPPlugin\GuzzleHttp\Handler\CurlShareHandleState;
...
class Client implements ClientInterface, \SMTP2GOWPPlugin\Psr\Http\Client\ClientInterface
{
    ...
    public function requestAsync(string $method, $uri = '', array $options = []) : PromiseInterface
    {
        ...
        if ($method !== $normalizedMethod) {
            \SMTP2GOWPPlugin\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing a non-uppercase HTTP method to Client::requestAsync() is deprecated; guzzlehttp/guzzle 8.0 will preserve HTTP method casing. Pass an uppercase method explicitly if uppercase is required.');
    ...
    public function request(string $method, $uri = '', array $options = []) : ResponseInterface
    {
        $normalizedMethod = \strtoupper($method);
        if ($method !== $normalizedMethod) {
            \SMTP2GOWPPlugin\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing a non-uppercase HTTP method to Client::request() is deprecated; guzzlehttp/guzzle 8.0 will preserve HTTP method casing. Pass an uppercase method explicitly if uppercase is required.');
            $method = $normalizedMethod;
        }
        $options[RequestOptions::SYNCHRONOUS] = \true;
    ...
    private static function warnAboutRequestLevelHandler(array $options) : void
    {
        ...
        \SMTP2GOWPPlugin\trigger_deprecation('guzzlehttp/guzzle', '7.12', 'Passing the "handler" request option is deprecated; guzzlehttp/guzzle 8.0 will ignore request-level handlers. Configure the handler when creating the Client, or use a separate Client instance for requests that need a different handler.');
    }
    ...
    {
        \SMTP2GOWPPlugin\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing %s to request option "%s" is deprecated; guzzlehttp/guzzle 8.0 requires %s.', \get_debug_type($value), $option, $expected);
    }
    /**
    ...
    private function transfer(RequestInterface $request, array $options) : PromiseInterface
    {
        ...
        if ('' === $protocolVersion) {
            \SMTP2GOWPPlugin\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Sending a request with an empty protocol version is deprecated; guzzlehttp/guzzle 8.0 will reject empty protocol versions.');
            $request = Psr7\Utils::modifyRequest($request, ['version' => '1.1']);
        } elseif (!self::isProtocolVersionValid($protocolVersion)) {
            \SMTP2GOWPPlugin\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Sending a request with a malformed protocol version is deprecated; guzzlehttp/guzzle 8.0 will reject malformed protocol versions.');
        }
        ...
        if (!empty($options['auth']) && \is_array($options['auth'])) {
            ...
            switch ($type) {
                ...
                case 'digest':
                    ...
                    break;
                case 'ntlm':
                    \SMTP2GOWPPlugin\trigger_deprecation('guzzlehttp/guzzle', '7.12', 'Passing "ntlm" as the built-in auth type is deprecated; guzzlehttp/guzzle 8.0 will no longer apply NTLM through the "auth" request option. Configure NTLM with cURL HTTP authentication options instead.');
    ...
    private static function createBodyStream($body) : StreamInterface
    {
        ...
        if (\is_scalar($body)) {
            \SMTP2GOWPPlugin\trigger_deprecation('guzzlehttp/guzzle', '7.12', 'Passing a non-string scalar to the "body" request option is deprecated; guzzlehttp/guzzle 8.0 will reject non-string scalar bodies.');
    ...
    private static function normalizeNonFiniteFloats(array $values, string $option) : array
    {
        foreach ($values as $key => $value) {
            if (\is_array($value)) {
                ...
                \SMTP2GOWPPlugin\trigger_deprecation('guzzlehttp/guzzle', '7.12', 'Passing a non-finite float in the "%s" request option is deprecated; guzzlehttp/guzzle 8.0 will reject non-finite floats.', $option);
    ...
    {
        if ('' === $version) {
            \SMTP2GOWPPlugin\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing an empty "version" request option is deprecated; guzzlehttp/guzzle 8.0 will reject empty protocol versions.');
            return '1.1';
        }
