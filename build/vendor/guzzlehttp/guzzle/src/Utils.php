...
use SMTP2GOWPPlugin\GuzzleHttp\Handler\CurlVersion;
...
final class Utils
{
    ...
    {
        \SMTP2GOWPPlugin\trigger_deprecation('guzzlehttp/guzzle', '7.12', '%s() is deprecated and will be removed in 8.0. Use get_debug_type() instead.', __METHOD__);
...
EOT
    ...
    public static function normalizeIdnConversionOption($value) : ?int
    {
        ...
        if (\is_string($value) && \is_numeric($value) || \is_float($value) && \is_finite($value)) {
            \SMTP2GOWPPlugin\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing %s as the "idn_conversion" request option is deprecated; guzzlehttp/guzzle 8.0 will reject values that are not true, false, null, or an integer IDNA_* bitmask.', \get_debug_type($value));
            return (int) $value;
        }
